        ...
        if ctx.settings.sqlite_path and Path(ctx.settings.sqlite_path).exists():
            ...
            with conn:
                ...
                try:
                    ...
                    pass
                base_q = "SELECT id as _id, keyword, author, author_profile, company, text, published_at, collected_at, permalink FROM posts"
                params: list[Any] = []
                # Base WHERE to exclude demo rows
                where_clauses = ["LOWER(author) <> 'demo_recruteur'", "LOWER(keyword) <> 'demo_recruteur'"]
                if q:
                    ...
                    try:
                        ...
                        if "search_norm" in cols:
                            ...
                            where_clauses.append("(search_norm LIKE ? OR (search_norm IS NULL AND (text LIKE ? OR author LIKE ? OR company LIKE ? OR keyword LIKE ?)))")
                            qn = _normalize_for_search(q)
                            pat_norm = f"%{qn}%"
                            pat_raw = f"%{q}%"
                            params.extend([pat_norm, pat_raw, pat_raw, pat_raw, pat_raw])
                        else:
                            where_clauses.append("(text LIKE ? OR author LIKE ? OR company LIKE ? OR keyword LIKE ?)")
                            pat = f"%{q}%"
                ...
                sqlite_field = sort_field
                dir_sql = "ASC" if sort_direction == 1 else "DESC"
                base_q += f" ORDER BY {sqlite_field} {dir_sql} LIMIT ? OFFSET ?"
                ...
                if q:
                    try:
                        ...
                        if "search_norm" in cols:
                            ...
                            pat_raw = f"%{q}%"
                            row = conn.execute(
                                "SELECT COUNT(*) FROM posts WHERE LOWER(author) <> 'demo_recruteur' AND LOWER(keyword) <> 'demo_recruteur' AND (search_norm LIKE ? OR (search_norm IS NULL AND (text LIKE ? OR author LIKE ? OR company LIKE ? OR keyword LIKE ?)))",
                                (pat_norm, pat_raw, pat_raw, pat_raw, pat_raw),
                            ).fetchone()
                        else:
                            pat = f"%{q}%"
                            row = conn.execute(
                                "SELECT COUNT(*) FROM posts WHERE LOWER(author) <> 'demo_recruteur' AND LOWER(keyword) <> 'demo_recruteur' AND (text LIKE ? OR author LIKE ? OR company LIKE ? OR keyword LIKE ?)",
                                (pat, pat, pat, pat),
                            ).fetchone()
                    except Exception:
                        pat = f"%{q}%"
                    ...
                    row = conn.execute("SELECT COUNT(*) FROM posts WHERE LOWER(author) <> 'demo_recruteur' AND LOWER(keyword) <> 'demo_recruteur'").fetchone()
...
async def login_page(request: Request, ctx=Depends(get_auth_context)):
    st = await session_status(ctx)
    return templates.TemplateResponse("login.html", {"request": request, "session": st.details, "valid": st.valid})
