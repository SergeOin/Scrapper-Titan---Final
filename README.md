...
<!-- Local coverage badge (fallback generated by scripts/generate_badge.py in CI artifacts) -->
<!-- If you want to embed the generated SVG directly (served from repo once committed), move coverage_badge.svg into docs/ and link it here. -->
...
### (Alternative) Makefile (Linux / macOS / Git Bash)
Si vous disposez de `make` :
```bash
make install           # deps runtime
make install-dev       # + deps dev (lint, tests)
make test              # tests rapides
make coverage          # coverage + badge local (coverage_badge.svg)
make lint              # ruff + mypy
make format            # ruff --fix + black
make show-config       # snapshot config safe
make server            # uvicorn
make worker            # worker seul
```
Sur Windows natif PowerShell, utiliser `tasks.ps1` (Invoke-Task) ou installer make (Git for Windows / choco).
...
python scripts/generate_badge.py  # génère coverage_badge.svg
```
Auto-fix format :
```powershell
ruff check . --fix
